    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            ragas_result = ragas_evaluate(
                ragas_input, metrics=[answer_relevancy, faithfulness]
            )

        scores = ragas_result.scores[0]
        if scores:
            metrics["ragas_answer_relevance"] = round(
                float(scores.get("answer_relevancy", 0.0)), 4
            )
            metrics["ragas_faithfulness"] = round(
                float(scores.get("faithfulness", 0.0)), 4
            )
        else:
...
def simple_faithfulness(predicted_answer, predicted_context):
    ...
    sims = [
        float(cosine_similarity([a_embedding], [c])[0][0]) for c in context_embeddings
    ]
...
def compute_overlap(predicted_context, expected_context, exact_match):
    """
    Сравнение retrieved vs expected context:
    - exact_match=True  → fuzzy string matching
    - exact_match=False → cosine similarity через SentenceTransformer
    """
    if exact_match:
        from difflib import SequenceMatcher
        ...
        matches = sum(
            1 for pc in predicted_context
            for ec in expected_context
            if fuzzy_match(pc, ec)
        )
        return matches, len(predicted_context), len(expected_context)

    # Семантическое сравнение через эмбеддинги
    if sentence_model is None:
...
def answer_correctness(predictions, references):
    ...
    return [
        SequenceMatcher(None, p.strip().lower(), r.strip().lower()).ratio()
        for p, r in zip(predictions, references)
    ]
